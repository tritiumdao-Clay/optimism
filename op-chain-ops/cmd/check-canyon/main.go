...
// If any error is encounted, valid is set to false.
func CheckActivation(f func(Args, bool) error, ctx Args, forkActivated bool, validatePreState bool, valid *bool) {
	if forkActivated {
		if err := f(ctx, true); validatePreState && err != nil {
...
func main() {
	...
	sourceClient, err := sources.NewL1Client(l1RPC, logger, nil, l1ClCfg)
	if err != nil {
		log.Crit("Error creating RPC", "err", err)
	}
	...
	CheckActivation(ValidateReceipts, ctx, canyonActive, true, &valid)
	CheckActivation(Validate1559Params, ctx, canyonActive, false, &valid)
	CheckActivation(ValidateWithdrawals, ctx, canyonActive, true, &valid)
	CheckActivation(ValidateCreate2Deployer, ctx, canyonActive, true, &valid)

	if !valid {
		os.Exit(1)
	} else if canyonActive {
		log.Info(fmt.Sprintf("Succesfully validated block %v as a Canyon block", number))
	} else {
		log.Info(fmt.Sprintf("Succesfully validated block %v as a Pre-Canyon block", number))
	}
}
