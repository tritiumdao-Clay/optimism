...
func checkPredeployConfig(client *ethclient.Client, name string) error {
	...
	g.Go(func() error {
		...
		case predeploys.SafeL2Addr:
			if err := checkPredeployBytecode(p, "SafeL2", client, []byte(bindings.SafeL2MetaData.ABI)); err != nil {
				return err
			}

		case predeploys.MultiSendAddr:
			if err := checkPredeployBytecode(p, "MultiSend", client, []byte(bindings.MultiSendMetaData.ABI)); err != nil {
				return err
			}

		case predeploys.MultiSendCallOnlyAddr:
			if err := checkPredeployBytecode(p, "MultiSendCallOnly", client, []byte(bindings.MultiSendCallOnlyMetaData.ABI)); err != nil {
				return err
			}

		case predeploys.Multicall3Addr:
			if err := checkPredeployBytecode(p, "Multicall3", client, []byte(bindings.Multicall3MetaData.ABI)); err != nil {
				return err
			}

		case predeploys.Create2DeployerAddr:
			if err := checkPredeployBytecode(p, "Create2Deployer", client, []byte(bindings.Create2DeployerMetaData.ABI)); err != nil {
				return err
			}

		case predeploys.Permit2Addr:
			if err := checkPredeployBytecode(p, "Permit2", client, []byte(bindings.Permit2MetaData.ABI)); err != nil {
				return err
			}

		case predeploys.EntryPointAddr:
			if err := checkPredeployBytecode(p, "EntryPoint", client, []byte(bindings.EntryPointMetaData.ABI)); err != nil {
				return err
			}
		}
		return nil
